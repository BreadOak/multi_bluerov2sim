...
def go_point(master,DesX,DesY,DesZ,DesYaw):
	...
	#for i in range (0, 1) :
	    ...
	    #print((inversefunc(pwm_2_thr,y_values = PID_X/9.8)))
	    #print('PID_pwm_X:',PID_pwm_X)
...
def get_imu_data(master):
	return master_0.recv_match(type='SCALED_IMU2', blocking=True)

def get_imu_matrix(master):
	msg = get_imu_data(master_0)
...
def rotation(Yaw):
	...
	while(1):
		...
		imu_string = str(get_imu_data(master_0))
		...
		print('imu_ang_y:',imu_ang_y)
		print(abs(abs(imu_ang_y) - abs(Yaw)))

		if abs(abs(imu_ang_y) - abs(Yaw)) < 0.03 :
			if Yaw < 0:
				master_0.mav.manual_control_send(master_0.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600*0.82, 0)
				time.sleep(3)
				master_0.mav.manual_control_send(master_0.target_system, 0, 0, 0, 0, 0)
			break
		else:
			#master_0.mav.manual_control_send(master_0.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600, 0)
			
			if Yaw < 0:
				master_0.mav.manual_control_send(master_0.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600*0.82, 0)
			else:
				master_0.mav.manual_control_send(master_0.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600*0.9, 0)
			
		
		st = time.time() - start_time
		#print('dd',(Yaw/abs(Yaw))*0.9)
...
if __name__ =='__main__':
	...
	#b0 = Process(target = go_point(master_0, -1, 0, 0, 0), args = () )
	#b1 = Process(target = go_point(master_1, -1, 0, 0, 0), args = () )
#b1.start()
#b0.start()
	'''
	rotation(-math.pi/2)
	go_point(master_0, 1, 0, 0, 0)
	rotation(math.pi/2)
