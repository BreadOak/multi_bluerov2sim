...
def rotation(Yaw):
	...
	while(1):
		...
		imu_string = str(get_imu_data(master_2))
		...
		if abs(abs(imu_ang_y) - abs(Yaw)) < 0.03 :
			if Yaw < 0:
				master_2.mav.manual_control_send(master_2.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600*0.82, 0)
				time.sleep(3)
				master_2.mav.manual_control_send(master_2.target_system, 0, 0, 0, 0, 0)
			break
		else:
			#master_2.mav.manual_control_send(master_2.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600, 0)
			
			if Yaw < 0:
				master_2.mav.manual_control_send(master_2.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600*0.82, 0)
			else:
				master_2.mav.manual_control_send(master_2.target_system, 0, 0, 0, (Yaw/abs(Yaw))*600*0.9, 0)
...
if __name__ =='__main__':
	...
	## connect
	master_2 = mavutil.mavlink_connection('udpin:0.0.0.0:14570')

	master_2.wait_heartbeat()

	## Arm
	master_2.mav.command_long_send(master_2.target_system, master_2.target_component, mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM, 0, 1, 0, 0, 0, 0, 0, 0)

	while(1):
		go_point(master_2, 1, 0, 0, 0)
		go_point(master_2, 1, 0, 0, 0)
		go_point(master_2, 1, 0, 0, 0)
		go_point(master_2, 1, 0, 0, 0)
		rotation(-math.pi/2)
